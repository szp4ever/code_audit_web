...
import { downloadDocument, getDocument } from '@/api/v2/document'
...
async function handleDownloadFile() {
	if (!props.task?.attachId && !props.task?.docId) {
		message.warning('附件ID不存在')
		return
	}
	try {
		let docId = props.task.docId
		if (!docId && props.task.attachId) {
			const info: any = await getDocument(props.task.attachId)
			docId = info?.data?.docId || info?.docId
		}
		if (!docId) {
...
function handleViewAttach() {
	if (props.task?.kid) {
		router.push({ path: '/knowledge-v2/' + props.task.kid, query: { tab: 'document' } })
		show.value = false
	}
}
</script>
